<?php
    ...
    if($_REQUEST['CRUD']=='C'){
        if($_REQUEST['USER_CD']==""){ //신규 작성
            $sql = "INSERT INTO EDU_FAMILY.EDU_USER(USER_ID,USER_NM,USER_PASS,USER_AUTH,EMAIL,POSITION,ORG_NM,REG_DT,MEMO) VALUES ('";
            ...
            $sql = "UPDATE EDU_FAMILY.EDU_USER SET 
                USER_ID='".$_REQUEST['USER_ID']."'
                ,USER_NM='".$_REQUEST['USER_NM']."'";
            if(strlen($_REQUEST['USER_PASS'])!=32){ //md5의 자리수인 32자리가 아닌 다른 자리수의 비번이라면 비번이 변경된 것이므로
        ...
        //기본 쿼리
        $sql = "SELECT * FROM EDU_FAMILY.EDU_USER";
        //조건문 지정
        $whereSql = " WHERE 1=1 ";
        if(@$_REQUEST['USER_CD']){
        ...
        //기본 쿼리
        $sql = "DELETE FROM EDU_FAMILY.EDU_USER WHERE USER_CD = '".$_REQUEST['USER_CD']."'";
        echo $sql; //오류 점검용 쿼리
        $result = mysqli_query($conn,$sql);
        mysqli_close($conn);
    }else{
