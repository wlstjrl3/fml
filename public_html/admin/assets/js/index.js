...
newCol.addEventListener("click",()=>{
    ...
    preDate.value="0"; //강좌 설정페이지는 별도 key값이 없기에 이 칸을 이용하여 신규인지 수정인지 판단한다.
});
//행을 클릭했을때 xhr로 다시 끌어올 데이터는 각 페이지마다 다르기에 여기에서 지정
function trDataXHR(idx){ 
    let xhr = new XMLHttpRequest();
    xhr.open("GET", "/admin/sys/eduConfig.php?key="+psnlKey.value+"&EDU_DT="+idx+"&CRUD=R"); xhr.send(); //XHR을 즉시 호출한다. psnlKey는 추후 암호화 하여 재적용 예정
    console.log("/admin/sys/eduConfig.php?key="+psnlKey.value+"&EDU_DT="+idx+"&CRUD=R");
    xhr.onload = () => {
                ...
                document.querySelector(".modalBody").querySelectorAll("input").forEach((input,key)=>{
                    switch(key){
                        case 0 :
                            ...
                            preDate.value=res[0].EDU_DT; //강좌 설정페이지는 별도 key값이 없기에 이 칸을 이용하여 신규인지 수정인지 판단한다.
                            break;
                        case 1 :
...
modalEdtBtn.addEventListener("click",()=>{
    ...
    let writeUrl='';
    document.querySelector(".modalForm").querySelectorAll("input").forEach((input,key)=>{
        if(key==0){writeUrl+="&EDU_DT="+input.value}
        else if(key==1){writeUrl+="&END_DT="+input.value}
        else if(key==2){writeUrl+="&EDU_PAY="+input.value}
        else if(key==3){writeUrl+="&TEAM_CNT="+input.value}
    });
    writeUrl+="&ONLINE="+document.querySelector(".modalBody").querySelector("select").value;
    writeUrl+="&preDate="+preDate.value; //강좌 설정페이지는 별도 key값이 없기에 이 칸을 이용하여 신규인지 수정인지 판단한다.
    ...
    if(fileName.slice(-4)=='xlsx'||fileName.slice(-4)=='.xls'){
        ...
        reader.onload = function () {
            ...
            workBook.SheetNames.forEach(function (sheetName) {
                ...
                if(rows[0]['교육일']){
                    if (!confirm("교육일:"+rows[0]['교육일']+"[교육비:"+rows[0]['교육비']+"] 참가팀수:"+rows[0]['참가팀수']+" 외 "+(rows.length-1)+"건 의 정보를 일괄등록 하시겠습니까?")) {
                        // 취소(아니오) 버튼 클릭 시 이벤트
                        return false;
                    }                     
                }else{
                ...
                xhr.open('POST', '/admin/sys/batchInsert.php?key=jsyang', true);//요청을 보낼 방식, 주소, 비동기여부 설정                
